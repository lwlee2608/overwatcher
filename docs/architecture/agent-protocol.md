...
Both endpoints authenticate with `Authorization: Bearer <AGENT_SHARED_SECRET>` and identify the agent with `X-Agent-Name: <name>`.
...
| `404` | `/result` for unknown intent id | log, continue |
